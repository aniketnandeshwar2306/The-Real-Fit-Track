...
export default function Dashboard() {
  ...
  const [modal, setModal] = useState(null) // 'profile' | 'meal' | 'water' | 'calories' | null
  ...
  const workoutProgress = Math.round((doneWorkouts / totalWorkouts) * 100)
    ...
    <div className="dashboard">
      ...
      <main className="dash-main">
        <div className="dash-header-row">
          ...
          <button className="btn btn-primary btn-sm" onClick={() => setModal('meal')}>+ New Workout</button>
        </div>
        ...
        <div className="stat-cards">
          ...
          <StatCard label="Workouts This Week" value={`${doneWorkouts} <span class="stat-total">/ ${totalWorkouts}</span>`}
            change={doneWorkouts === totalWorkouts ? 'All done! 🎉' : `${totalWorkouts - doneWorkouts} more to go!`}
        ...
        <div className="dash-grid-2">
          <QuickActions
            onWorkout={() => showToast('Click exercises above to mark them done')}
